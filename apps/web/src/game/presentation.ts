...
export function presentation(
  ...
  for (const event of events) {
    ...
    if (
      ['payment', 'income', 'start_bonus', 'sale'].includes(event.type) &&
      (event.amount ?? 0) > 0
    ) {
      ...
      add({
        ...
        tile: event.tile,
        sound: 'payment',
        duration: reduced ? 350 : 1200,
      });
    }
    if (['purchase', 'build', 'championship'].includes(event.type)) {
      const payer = visual.players.find((p) => p.id === event.playerId);
      if (payer) payer.cash = Math.max(0, payer.cash - (event.amount ?? 0));
    }
