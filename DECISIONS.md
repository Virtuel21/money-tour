...
Avant le premier tour complet validé par un passage Départ en avant, le niveau maximal est 2. Ensuite, le niveau 3 devient accessible ; l'hôtel est accessible depuis le niveau 3, y compris par deux améliorations successives lors d'une même visite si le budget le permet. Chaque amélioration ne fait avancer que d'un niveau. Depuis la révision 3D demandée par Julien, posséder personnellement toutes les villes du groupe est obligatoire pour construire. Un coéquipier ne complète pas le groupe pour cette permission. Cela ne multiplie pas automatiquement le loyer.
...
## Révision cartoon 3D — 26 septembre 2026

La demande de Julien remplace le rendu 2D par des modèles Blender originaux exportés en glTF et affichés avec Three.js. Le moteur conserve la décision aléatoire ; les dés 3D ne calculent pas un nouveau résultat physique. Ils roulent et rebondissent pendant 1,6 seconde puis se posent sur les valeurs validées. Le pion avance ensuite par bonds de 270 ms, case par case, y compris dans les déplacements arrière. Les cartes restent au centre jusqu’au clic pour un humain en local ; pour les bots et en ligne, elles apparaissent pendant 5,5 secondes avec un bouton permettant de continuer ; elles restent consultables dans le panneau de jeu. Les constructions émergent sur 700 ms. Les réglages de mouvement réduit raccourcissent ces transitions.

Les animations sont une file de présentation distincte de l’état réseau. Les bots en ligne attendent la durée de présentation avant leur prochaine décision ; le chrono réseau continue. En local, le chrono attend les animations et la lecture des cartes. Les versions de configuration et de salon passent en v2 pour éviter de mélanger les règles de construction. Les anciennes sauvegardes incompatibles ne sont pas reprises automatiquement.

Embarquer crée directement un salon privé depuis l’accueil en conservant nom, nombre de sièges, durée et équipes. Le jeu solo/local possède son bouton distinct. La copie du lien et du code est proposée dans le salon.

Les deux MP3 fournis représentent déjà 6,3 Mo ; la limite totale des assets passe explicitement de 5 à 12 Mo pour les conserver sans dégradation audio, avec préchargement musical désactivé. Les autres assets sont des GLB, WebP et petits échantillons OGG CC0.
