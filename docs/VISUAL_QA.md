...
## Édition 8 — Mondial et règles variables

302 tests réussis, dont les tests réseau à deux sessions avec des offres de 23 456 et 34 567, victoire de la seconde et débit du seul gagnant. Cinq variantes simulées, objectifs récompensés une seule fois, classement de Capitale après paiement, enchère du Marché au tour de table 10, paiements, délais et reprise v7 couverts. Couverture moteur : 97,93 % des lignes, 93,87 % des branches, 100 % des fonctions.

Contrôle visuel local : clic sur Lisbonne depuis la case Mondial → 50 k débités, loyer de 10 à 20 k, badge quatre tours. 30 cases assombries et deux villes éligibles visibles. Le panneau de sélection reste à droite et les cartes des joueurs ne sont plus comprimées par le conteneur. Enchère locale avec offre 123 456 : le champ est effacé avant le joueur suivant, les trois autres passent, Léa obtient Madrid avec 1 376 544 en banque. Annonce sans montant, temps de lecture prolongé et pause du décompte. Les scènes utilisent le harness de développement, absent des routes de jeu publiques.
